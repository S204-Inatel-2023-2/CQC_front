...
export const Conexoes = ({navigation}, props) =>{
  ...
  const buscaHandle = (text) =>{
    setBuscaInput(text)
    const session = driver.session();
    setCarregarComponente(false);
    
    const query = "MATCH (n:Usuario) WHERE n.nome CONTAINS $nome AND NOT n.email = $email RETURN n";
    console.log(text)
      session
            .run(query,{nome:text,email: listaConexao[0][0]['email']})
            .then(result => {
  ...
  const  minhaFuncao = () => {
    const session = driver.session();
     const query = "MATCH (n:Usuario) WHERE NOT n.email = $email RETURN n "
       session
    ...
    return(
      ...
      <View style={styles.container1}>
                    <TextInput  value={buscaInput} onChangeText={(text)=>buscaHandle(text)} style={styles.input} placeholder='Digite um nome para buscar'></TextInput>
                  </View>
